...
app.commandLine.appendSwitch("js-flags", "--max-old-space-size=16096");
...
app.on("ready", function () {
  ...
  browserWindow.once("ready-to-show", function () {
    ...
    ipcMain.on("frameReady", function (event, arg) {
      ...
      browserWindow
        .capturePage({ x: 0, y: 0, width: width, height: height })
        .then(function (img) {
          var file =
            outputDirectory + "/" + ("00000" + frame++).slice(-5) + ".png";
          return fsExtra.outputFile(file, img.toPNG()).then(function () {
